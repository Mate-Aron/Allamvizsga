...
if (!$provided_user || ($provided_user !== $BASIC_AUTH_USER || !password_verify($provided_pw, $BASIC_AUTH_PASS_HASH))) {
    ...
    die('<h1>Access Denied</h1><p>Kérlek jelentkezz be.</p>');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($posted_token) || !hash_equals($_SESSION['csrf_token'], $posted_token)) {
        $action_result = '<div class="error">Security Error: Invalid CSRF Token. Refresh the page!</div>';
    } else {
        $action = $_POST['action'] ?? '';
        ...
        if (!is_writable($WHITELIST_FILE) && !in_array($action, ['save_file'])) {
            ...
            if ($action === 'disable_target') {
                ...
                if (atomic_append_file($WHITELIST_FILE, $line)) {
                    ...
                    $action_result = "<div class='error'>Save failed.</div>";
                }
                ...
                if (file_exists($WHITELIST_FILE)) {
                    ...
                    foreach ($lines as $line) {
                        if (strpos($line, $source_ip) !== false && strpos($line, "ruleRemoveById=$rid") !== false) {
                            $found = true;
                            continue; 
                        }
                        $new_lines[] = $line;
                    ...
                    if ($found) {
                        $new_content = implode("\n", $new_lines) . "\n";
                        if (file_put_contents($WHITELIST_FILE, $new_content) !== false) {
                            $action_result = "<div class='success'>↩ Whitelist revoked: $source_ip (Rule: $rid). ⚠️ Apache reload required!</div>";
                        } else {
                            $action_result = "<div class='error'>Save failed. No write permission?</div>";
                        }
                    } else {
                        $action_result = "<div class='error'>This rule is no longer in the whitelist.</div>";
                    }
                }
                ...
                if (file_exists($WHITELIST_FILE)) {
                    ...
                    if ($found) {
                        $new_content = implode("\n", $new_lines) . "\n";
                        if (file_put_contents($WHITELIST_FILE, $new_content) !== false) {
                            $action_result = "<div class='success'>↩ Global whitelist revoked (Rule: $rid). ⚠️ Apache reload required!</div>";
                        } else {
                            $action_result = "<div class='error'>Save failed. No write permission?</div>";
                        }
                    } else {
                            $action_result = "<div class='error'>This rule is not globally whitelisted.</div>";
                        }
                    }
                } 
            elseif ($action === 'save_file') {
                ...
                if (allowed_local_path(basename($path)) && save_rule_text($path, $content)) {
                    ...
                    $action_result = "<div class='error'>Save failed. Permission error?</div>";
                }
...
<div class="dashboard-container">
    ...
    <div id="live-logs-wrapper">
        <?php if (empty($logs)): ?>
            <div class="info-box">The file is not readable: <code><?= h($AUDIT_LOG) ?></code></div>
    ...
    <div class="log-card <?= $card_class ?>">
        ...
        <?php break; case 'rules': 
                        ...
                        foreach ($activated_files as $f) {
                            foreach (parse_rules_from_file($f) as $rule) {
                        ...
                        $whitelisted_rules = [];
                        if (file_exists($WHITELIST_FILE)) {
                            $whitelisted_rules = parse_rules_from_file($WHITELIST_FILE);
                        }
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function fetchNewLogs() {
        ...
        fetch(window.location.href)
            ...
            .catch(err => console.error('Hiba a logok frissítésekor:', err));
    }
